...
fn resample_to_16k_mono(samples: &[f32], source_hz: u32) -> Vec<f32> {
    ...
    let mut resampler =
        FrameResampler::new(source_hz as usize, WHISPER_SAMPLE_RATE as usize, Duration::from_millis(30));
    let mut out = Vec::with_capacity(samples.len() * WHISPER_SAMPLE_RATE as usize / source_hz as usize);
...
pub fn run_import(
    ...
    for (chunk_index, chunk) in samples.chunks(chunk_samples.max(1)).enumerate() {
        ...
        let end_ms = start_ms
            + (chunk.len() as u64 * 1000 / WHISPER_SAMPLE_RATE as u64).max(1);
...
mod tests {
    ...
    fn import_manager_rejects_concurrent_imports() {
        ...
        assert!(guard2.is_none(), "second concurrent import should be rejected");

        drop(guard1);
        let guard3 = manager.try_start();
        assert!(guard3.is_some(), "slot should be free after guard is dropped");
    }
