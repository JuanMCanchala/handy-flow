...
pub fn classify(process_name: &str, window_title: &str) -> StyleCategory {
    ...
    if WORK_PROCESSES.iter().any(|p| contains_ci(&process_lower, p)) {
...
pub fn instruction_for(tone_key: Option<&str>) -> Option<&'static str> {
    tone_key.and_then(StyleTone::from_key).map(|t| t.instruction())
}
...
mod platform {
    ...
    pub fn detect_foreground_app() -> Option<ForegroundApp> {
        ...
        let process_name = app.localizedName().map(|s| s.to_string()).unwrap_or_default();
...
mod tests {
    ...
    fn classifies_personal_messaging_apps() {
        assert_eq!(classify("WhatsApp.exe", "WhatsApp"), StyleCategory::Personal);
    ...
    fn classifies_work_messaging_apps() {
        ...
        assert_eq!(classify("Teams.exe", "Microsoft Teams"), StyleCategory::Work);
    }
