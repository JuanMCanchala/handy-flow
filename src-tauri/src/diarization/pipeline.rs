...
pub fn diarize(
    ...
    let seg_path =
        model_manager.model_path(crate::diarization::models::DiarizationModelKind::Segmentation)?;
    let emb_path =
        model_manager.model_path(crate::diarization::models::DiarizationModelKind::Embedding)?;
...
mod tests {
    ...
    fn real_models_diarize_a_multi_speaker_recording() {
        ...
        let mut segmentation =
            crate::diarization::onnx::SegmentationModel::load(std::path::Path::new(&seg_path))
                .expect("load segmentation model");
        let mut embedding =
            crate::diarization::onnx::EmbeddingModel::load(std::path::Path::new(&emb_path))
