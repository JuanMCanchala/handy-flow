...
impl SegmentationModel {
    pub fn load(path: &Path) -> Result<Self> {
        ...
        session_builder = ort_ctx!(
            session_builder.with_intra_threads(1),
            "ort intra threads"
        )?;
        let session = ort_ctx!(
    ...
    pub fn segment(&mut self, samples: &[f32]) -> Result<Vec<RawSpeakerSpan>> {
        ...
        for (i, chunk_labels) in labels.iter().enumerate() {
            let start = ((i * window_shift) as f64 / self.meta.receptive_field_shift as f64
                + 0.5) as usize;
                ...
                if is_active {
                    if frame_activity[i] < OFFSET {
                        if let Some(sf) = start_frame {
                            push_span(&mut spans, sf, i, speaker, scale, scale_offset, MIN_DURATION_ON);
                        }
                        is_active = false;
                        start_frame = None;
                    }
    ...
    fn run_segmentation(&mut self, batch: &Array3<f32>) -> Result<Array3<f32>> {
        ...
        let outputs = ort_ctx!(self.session.run(inputs!["x" => input]), "segmentation inference")?;
        let y = ort_ctx!(
...
impl EmbeddingModel {
    ...
    pub fn embed(&mut self, samples: &[f32]) -> Result<Option<Vec<f32>>> {
        ...
        let outputs = ort_ctx!(self.session.run(inputs!["feats" => input]), "embedding inference")?;
