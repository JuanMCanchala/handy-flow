...
impl DiarizationModelManager {
    ...
    pub async fn download(&self, kind: DiarizationModelKind) -> Result<()> {
        ...
        let final_bytes = match &spec.source {
            ...
            ModelSource::TarBz2 { entry_path } => extract_from_tar_bz2(&downloaded, entry_path)
...
mod tests {
    ...
    fn models_status_reports_both_kinds() {
        let dir =
            std::env::temp_dir().join(format!("handy_diarization_test_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let manager = DiarizationModelManager {
    ...
    fn is_downloaded_reflects_file_presence() {
        let dir =
            std::env::temp_dir().join(format!("handy_diarization_test2_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let manager = DiarizationModelManager {
            models_dir: dir.clone(),
        };
